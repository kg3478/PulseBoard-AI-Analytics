...
export default function UploadPage({ onUploadSuccess }) {
  ...
  useEffect(() => {
    if (readyToNavigate) navigate('/dashboard');
  ...
  const handleFile = useCallback(async (file) => {
    ...
    try {
      ...
      const sessionData = {
        ...
        csvContent: result.csv_content, // stored client-side for stateless API calls
      };
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-2xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/30 rounded-full px-4 py-1.5 text-indigo-400 text-sm font-medium mb-6">
            <Sparkles size={14} /> AI-Powered Analytics · Under 4 Minutes
          </div>
          <h1 className="text-5xl font-bold mb-4 leading-tight">
            Ask questions of your data<br /><span className="text-gradient">in plain English.</span>
          </h1>
          <p className="text-slate-400 text-lg leading-relaxed">
            Upload your CSV. Type a question. Get a chart instantly.<br />No SQL. No setup. No analyst required.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="w-full max-w-xl">
          {!uploadResult ? (
            <div
              onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
              onDragLeave={() => setDragOver(false)}
              onDrop={onDrop}
              className={`relative glass-card gradient-border p-10 text-center transition-all duration-300 cursor-pointer group ${dragOver ? 'border-indigo-500/60 bg-indigo-500/5 scale-[1.02]' : 'hover:border-white/10'} ${uploading ? 'pointer-events-none' : ''}`}
            ...
            >
              ...
              {uploading ? (
                ...
                <div className="flex flex-col items-center gap-4">
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300 ${dragOver ? 'bg-indigo-500/20 scale-110' : 'bg-navy-700 group-hover:bg-indigo-500/10'}`}>
                  ...
                  <div>
                    <p className="text-white font-semibold text-lg">{dragOver ? 'Drop it here!' : 'Drop your CSV here'}</p>
                    <p className="text-slate-400 text-sm mt-1">or click to browse · Max 50MB</p>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-slate-600 mt-2">
                    <FileSpreadsheet size={12} /><span>Stripe · Google Sheets · Airtable · Shopify</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card gradient-border p-6">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm">{uploadResult.schema.row_count?.toLocaleString()} rows · {uploadResult.schema.columns?.length} columns</p>
                </div>
              </div>

              <div className="flex flex-wrap gap-2 mb-5">
                {uploadResult.schema.columns?.map((col) => (
                  <span key={col.name} className={`text-xs px-2.5 py-1 rounded-lg border font-mono ${col.type === 'numeric' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : col.type === 'date' ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400' : 'bg-slate-700/50 border-white/10 text-slate-400'}`}>
                    {col.name} <span className="opacity-60">·{col.type[0].toUpperCase()}</span>
                  </span>
                ))}
              </div>

              <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-3">Suggested questions</p>
              <div className="space-y-2 mb-5">
                {uploadResult.starter_questions?.map((q, i) => (
                  <button key={i} onClick={() => setReadyToNavigate(true)} className="w-full text-left chip flex items-center gap-2">
                    <Sparkles size={12} className="text-indigo-400 flex-shrink-0" /><span>{q}</span>
                  </button>
                ))}
              </div>

              <button id="start-analyzing-btn" onClick={() => setReadyToNavigate(true)} className="btn-primary w-full flex items-center justify-center gap-2">
          ...
          {error && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 mt-4 text-rose-400 text-sm bg-rose-500/5 border border-rose-500/20 rounded-xl px-4 py-3">
              <AlertCircle size={16} />{error}
            </motion.div>
          )}
        </motion.div>
      </div>
      <div className="text-center py-6 text-slate-700 text-xs">PulseBoard · NL-to-SQL powered by Gemini</div>
    </div>
  );
}
